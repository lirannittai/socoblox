...
local function tweenBoxTo(id, toCell, duration)
	...
	local tw = TweenService:Create(ghost, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {CFrame = targetCF})
	tw:Play()
	tw.Completed:Connect(function()
		if ghost then ghost:Destroy() end
		if real then real.LocalTransparencyModifier = 0 end
	end)
end
...
local function toDir(inputState, inputObj)
	...
	local dir
	if key == Enum.KeyCode.W or key == Enum.KeyCode.Up then dir = Vector2.new(0,-1)
	elseif key == Enum.KeyCode.S or key == Enum.KeyCode.Down then dir = Vector2.new(0,1)
	elseif key == Enum.KeyCode.A or key == Enum.KeyCode.Left then dir = Vector2.new(-1,0)
	elseif key == Enum.KeyCode.D or key == Enum.KeyCode.Right then dir = Vector2.new(1,0) end
	if dir then Remotes.PushBoxRequest:FireServer(dir) end
end

ContextActionService:BindAction("SokobanMove", toDir, false,
	...
	elseif kind == "RoundEnd" then
		...
		if gui then
			...
			if share and share:FindFirstChild("ShareButton") then
				...
				share:SetAttribute("ShareText", ("I won %s on %s in %.2fs! Come beat my time!")
					:format(payload.levelId or "", Constants.GAME_NAME, (payload.ms or 0)/1000))
			end
		end
